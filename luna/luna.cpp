...
bool _lua_set_fence(lua_State* L, const char fence[]) {
    lua_getfield(L, LUA_REGISTRYINDEX, s_fence); // nullptr
    if (!lua_istable(L, -1)) {
        lua_pop(L, 1); // 
        lua_newtable(L); // L
        lua_pushvalue(L, -1); // L, L
        lua_setfield(L, LUA_REGISTRYINDEX, s_fence); // L
    }

    if (lua_getfield(L, -1, fence) != LUA_TNIL) { // L, nullptr
        lua_pop(L, 2);
        return false;
    }  
         
    lua_pushboolean(L, true); // L, nullptr, true
    lua_setfield(L, -3, fence); // L {"__fence__":true}, nullptr
    lua_pop(L, 2); //
    return true;     
}
